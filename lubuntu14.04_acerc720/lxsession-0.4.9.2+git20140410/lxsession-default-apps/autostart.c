...
static void ____lambda2_ (Block3Data* _data3_);
void ldefault_apps_update_autostart_conf (const gchar* line, const gchar* action, GtkBuilder* builder);
static void _____lambda2__gtk_toggle_button_toggled (GtkToggleButton* _sender, gpointer self);
static void ____lambda3_ (Block3Data* _data3_);
static void _____lambda3__gtk_button_clicked (GtkButton* _sender, gpointer self);
...
void ldefault_apps_manual_autostart_init (GtkBuilder* builder) {
	...
	if (_tmp4_) {
		...
		while (TRUE) {
			...
			g_signal_connect_data ((GtkToggleButton*) _tmp35_, "toggled", (GCallback) _____lambda2__gtk_toggle_button_toggled, block3_data_ref (_data3_), (GClosureNotify) block3_data_unref, 0);
			_tmp36_ = hbox;
			...
			button = _tmp38_;
			_tmp39_ = button;
			g_signal_connect_data (_tmp39_, "clicked", (GCallback) _____lambda3__gtk_button_clicked, block3_data_ref (_data3_), (GClosureNotify) block3_data_unref, 0);
			_tmp40_ = hbox;
			_tmp41_ = button;
		...
		_tmp55_ = add_button;
		g_signal_connect_data (_tmp55_, "clicked", (GCallback) ____lambda4__gtk_button_clicked, block2_data_ref (_data2_), (GClosureNotify) block2_data_unref, 0);
		_tmp56_ = auto_vbox;
...
void ldefault_apps_update_autostart_conf (const gchar* line, const gchar* action, GtkBuilder* builder) {
	...
	}
	{
		...
		GDataOutputStream* _tmp32_ = NULL;
		GDataOutputStream* _tmp33_ = NULL;
		FILE* _tmp34_ = NULL;
		gchar* read = NULL;
		...
		while (TRUE) {
			...
			_tmp38_ = read;
			g_message ("autostart.vala:196: read : %s", _tmp38_);
			_tmp39_ = line;
			g_message ("autostart.vala:197: line : %s", _tmp39_);
			_tmp40_ = read;
			_tmp41_ = line;
			if (g_strcmp0 (_tmp40_, _tmp41_) == 0) {
				GDataOutputStream* _tmp42_ = NULL;
		...
		if (g_strcmp0 (_tmp48_, "add") == 0) {
			GDataOutputStream* _tmp49_ = NULL;
	...
	__catch3_g_error:
	{
		...
		g_message ("autostart.vala:221: %s", _tmp57_);
		_g_error_free0 (e);
	}
	__finally3:
	if (_inner_error_ != NULL) {
